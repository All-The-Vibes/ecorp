...
function fixture(t) {
  ...
  function scan(config = policy) {
    ...
    try {
      ...
      return { nativeStatus: native.status, reporterStatus: reporter.status, output: JSON.parse(output) }
    } finally {
  ...
  return { repository, root, marker, commit, scan, git }
}
...
for (const historical of [false, true]) {
  test(`native scanner attributes a ${historical ? 'historical' : 'current'} finding and preserves failure`, { skip }, t => {
    ...
    assert.deepEqual(result.output.findings, [{
      commit: findingCommit, file: filename, start_line: 2, end_line: 2,
      rule_id: 'native-fixture', fingerprint: `${findingCommit}:${filename}:native-fixture:2`,
    }])
  })
}
