...
function Get-U1GitConfigFiles([string]$Product) {
    ...
    # Inspect every object entry without following directory aliases. Git can
    # load packed or loose objects through a junction without using alternates.
    ...
    while ($pending.Count) {
        foreach ($entry in Get-ChildItem -LiteralPath $pending.Pop() -Force -ErrorAction Stop) {
            if ($entry.Attributes -band [IO.FileAttributes]::ReparsePoint) {
                throw 'Reparse points are not supported in the source object database.'
            }
