...
$state = if (Test-Path -LiteralPath $statePath) { Get-Content -LiteralPath $statePath -Raw | ConvertFrom-Json -AsHashtable } else { @{ processes=@{}; previous=@(); restarts=@(); factory_enabled=$false } }
...
if ($Action -eq 'start') {
    ...
    foreach ($name in @($databaseName,$journalName)) {
        $found = & docker exec $container psql -U postgres -d postgres -Atc "SELECT 1 FROM pg_database WHERE datname='$name';"
        if ($LASTEXITCODE -ne 0) { throw 'Owned PostgreSQL unavailable.' }
        if ($found -ne '1') {
            & docker exec $container createdb -U postgres $name *> $null
    ...
    Launch 'anvil' (Get-Command pwsh.exe).Source @('-NoProfile','-File',(Join-Path $PSScriptRoot 'start_base_registry_anvil.ps1'),'-Port','18557','-ChainId','84532','-CacheDirectory',(Join-Path $hostRoot 'anvil-cache')) $hostRoot @{}
    Wait-Ready 'anvil' {
    ...
    Retain-AnvilChild
    Launch 'github' $node @((Join-Path $PSScriptRoot 'native_qualification_github.mjs')) $hostRoot @{CRONY_NATIVE_QUALIFICATION='1';CRONY_NATIVE_OUTPUT=$output}
...
$serverEnvironment = Get-Content (Join-Path $hostRoot 'server-secrets.json') -Raw | ConvertFrom-Json -AsHashtable
