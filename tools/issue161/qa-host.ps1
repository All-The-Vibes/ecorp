...
param(
    ...
    [string]$QaRoot = 'C:\Users\aabdelsalam\.ecorp\qa\issue-161-shared-authority',
    [string]$PgBin = 'C:\Users\aabdelsalam\.ecorp\pg\pgsql\bin',
...
function Set-PgEnvironment([string]$Database) {
    ...
    Remove-Item Env:\PGPASSWORD,Env:\PGSERVICE,Env:\PGSERVICEFILE,Env:\PGPASSFILE -ErrorAction SilentlyContinue
}
function Assert-Pg {
...
switch ($Action) {
    'InitPg' {
        ...
        & (Join-Path $PgBin 'initdb.exe') -D $pgData -U ecorp_qa161 -A trust --encoding=UTF8 --locale=C
    ...
    'DbTests' {
        Assert-Pg
        $env:DATABASE_URL = 'postgres://ecorp_qa161@127.0.0.1:55461/issue161_sqlx_maintenance'
