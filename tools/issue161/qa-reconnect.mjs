...
async function host(dry) {
  await run(process.env.ECORP_QA_PWSH,['-NoProfile','-NonInteractive','-File',path.join(root,'tools/issue161/qa-host.ps1'),
...
const restart=await run(process.execPath,[path.join(root,'tools/issue161/qa-api.mjs'),'restart-shared'],{
