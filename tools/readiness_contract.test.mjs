...
test('native test discovery covers existing web, tooling and steward suites', () => {
...
test('standalone live and historical replays stay separate while adjacent unit suites remain enrolled', () => {
  const files = [
    'tools/fixtures/delegated-keycloak/test/live.test.mjs',
    'tools/fixtures/delegated-keycloak/test/security.test.mjs',
    'tools/fixtures/pr226-evidence-baseline/check_evidence_personal_paths.test.mjs',
    'tools/fixtures/pr226-evidence-baseline/nested/sample.test.mjs',
    'tools/fixtures/pr226-evidence-baseline-next/sample.test.mjs',
    '.github/skills/code-review/tests/package.test.mjs',
  ]
  assert.deepEqual(selectNodeTests(files), [
    '.github/skills/code-review/tests/package.test.mjs',
    'tools/fixtures/delegated-keycloak/test/security.test.mjs',
    'tools/fixtures/pr226-evidence-baseline-next/sample.test.mjs',
  ])
...
test('empty discovery, invalid config and unknown groups fail closed', () => {
  ...
  const settings = { schemaVersion: 1, nodeTestRoots: ['tools/'], nodeTestSuffixes: ['.test.mjs'] }
  for (const nodeTestExcludes of ['tools/', [null], ['tools/../'], ['elsewhere/test.test.mjs']]) {
    assert.throws(() => selectNodeTests([], { ...settings, nodeTestExcludes }), /Unsupported/)
  }
})
...
const settings = { nodeTestRoots: ['tools/'], nodeTestExcludes: ['tools/fixtures/live.test.mjs'], rustCommand: ['cargo', 'test'], ignoredTests: 'Ignored is not passed.' }
