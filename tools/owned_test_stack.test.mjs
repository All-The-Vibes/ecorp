...
import { assertOwnedRestart, assertTestEndpoint, captureOwnedTestServerManifest, parseOwnedTestServerManifest, ownedServerEnvironment, restartOwnedTestServer, startOwnedTestServer, stopOwnedTestServer, verifyOwnedTestProcess } from './owned_test_stack.mjs'
...
test('native owned child startup, two restarts, refusal of database drift and idempotent stop', {
  ...
  try {
    ...
    const originalReceipt = readFileSync(manifestPath, 'utf8')
    await verifyOwnedTestProcess({ ...settings, manifest: initial })
    // The parent owns this listener, so the child has no listener at this endpoint.
    // Only explicit outbound admission may accept the matching child identity.
    const outbound = net.createServer()
    await new Promise(resolve => outbound.listen(0, '127.0.0.1', resolve))
    try {
      const outboundServer = `http://127.0.0.1:${outbound.address().port}`
      const outboundManifest = { ...initial, server_url: outboundServer }
      const admission = { ...settings, server: outboundServer, manifest: outboundManifest }
      await assert.rejects(verifyOwnedTestProcess(admission), /refusing/u)
      await verifyOwnedTestProcess({ ...admission, requireListener: false })
      for (const changed of [
        { ...outboundManifest, test_owned: false },
        { ...outboundManifest, workspace: path.join(folder, 'other') },
        { ...outboundManifest, server_creation: new Date(0).toISOString() },
        { ...outboundManifest, server_state: 'stopped' },
      ]) await assert.rejects(verifyOwnedTestProcess({ ...admission, requireListener: false, manifest: changed }), /refusing/u)
      await assert.rejects(verifyOwnedTestProcess({ ...admission, requireListener: false, binary: manifestPath }), /refusing/u)
      assert.equal(readFileSync(manifestPath, 'utf8'), originalReceipt, 'read-only admission leaves receipts unchanged')
      assert.equal((await fetch(`${endpoint}/health`)).ok, true, 'admission never stops the child')
      if (process.platform === 'win32') {
        const changed = { ...outboundManifest, server_identity: { ...initial.server_identity,
          native_creation_ticks: (BigInt(initial.server_identity.native_creation_ticks) + 1n).toString() } }
        await assert.rejects(verifyOwnedTestProcess({ ...admission, requireListener: false, manifest: changed }), /refusing/u)
      }
    } finally {
      await new Promise(resolve => outbound.close(resolve))
    }
