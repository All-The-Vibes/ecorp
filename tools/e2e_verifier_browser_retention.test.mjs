...
import { chmod, copyFile, mkdir, mkdtemp, readFile, readdir, realpath, stat, writeFile } from 'node:fs/promises'
...
for (const scenario of ['late-failure', 'copy-failure', 'stream-retention-failure',
  ...
  test(`driver evidence retention: ${scenario}`, async (t) => {
    ...
    const root = await realpath(await mkdtemp(path.join(parent, `F02-${scenario}-`)))
    ...
    await writeFile(path.join(modulePath, 'index.mjs'),
      'export const chromium = { executablePath: () => process.env.F02_EXECUTABLE }\n')
