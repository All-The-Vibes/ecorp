...
& (Join-Path $pg 'initdb.exe') -D (Join-Path $qa 'database') -U pr265_qa --auth=trust --encoding=UTF8 --locale=C
...
& (Join-Path $pg 'createdb.exe') -h 127.0.0.1 -p $DatabasePort -U pr265_qa pr265_activity
if ($LASTEXITCODE) { throw 'Could not create fresh acceptance database.' }
$serverEnv = @{
    DATABASE_URL="postgres://pr265_qa@127.0.0.1:$DatabasePort/pr265_activity"
