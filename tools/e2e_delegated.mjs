...
async function job() {
  const request = { actor_id: alice, room_id: room, idempotency_key: randomUUID() };
  const ids = await api(`${prefix}/jobs`, request);
  // A retry after the first response returns the same mission/task, including
  // while the runner is starting. The assignment identity is checked below.
  assert.deepEqual(await api(`${prefix}/jobs`, request), ids);
  const op = await until(async () => { const o = await operation(ids.task_id); return o?.run_id && o; });
  assert.deepEqual(await api(`${prefix}/jobs`, request), ids);
  const replay = await operation(ids.task_id);
  assert.equal(replay.id, op.id);
  assert.equal(replay.run_id, op.run_id);
  assert.equal(Number(sql(`SELECT count(*) FROM runs WHERE task_id='${uuid(ids.task_id)}'`)), 1);
  evidence.jobs.push({ ...ids, operation_id: op.id, run_id: op.run_id, idempotent_replay: true });
  return op;
}
function scope(op) {
...
async function browserLifecycle(config) {
  ...
  try {
    ...
    const admissions = [];
    await context.route('**/delegated/jobs', async route => {
      const request = route.request().postDataJSON();
      const response = await route.fetch();
      admissions.push(request);
      if (admissions.length === 1) await route.abort('failed');
      else await route.fulfill({ response });
    });
    const lostPopupPromise = context.waitForEvent('page');
    await runButton.click();
    const lostPopup = await lostPopupPromise;
    await panel.getByRole('alert').filter({ hasText: 'Could not confirm job creation' }).waitFor();
    await until(async () => lostPopup.isClosed());
    await page.reload();
    await until(async () => runButton.isEnabled());
    const popupPromise = context.waitForEvent('page');
    await runButton.click();
    const popup = await popupPromise;
    await until(async () => admissions.length === 2);
    assert.deepEqual(admissions[1], admissions[0]);
    await context.unroute('**/delegated/jobs');
    ...
    assert.deepEqual(await api(`${prefix}/jobs`, admissions[0]), { mission_id: op.mission_id, task_id: op.task_id });
    assert.equal(Number(sql(`SELECT count(*) FROM runs WHERE task_id='${uuid(op.task_id)}'`)), 1);
    assert.equal((await api(`${prefix}?actor_id=${alice}`)).operations.filter(candidate => !before.has(candidate.id)).length, 1);
    evidence.checks.push('browser-lost-create-response-reload-recovers-one-job-and-completed-replay-preserves-one-run');
