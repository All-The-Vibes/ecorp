...
async fn create_job(
    ...
    let plan = source.as_ref().map(protected_plan);
    ...
    // A replay recovers the durable admission even while its original runner
    // is disconnected. Scheduling is fenced by the persisted task/attempt state.
    if !replayed && !outcome.failures.is_empty() {
...
async fn persist_job(
    ...
    // Serialize retries before mission staffing or durable event creation. The
    // unique scoped key remains the database backstop; legacy rows keep NULL.
    sqlx::query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))")
        .bind(format!(
            "delegated-job:{corp}:{actor}:{}",
    ...
    if let Some(existing) = sqlx::query(
        "SELECT o.mission_id,o.task_id,o.issuer,o.subject,m.room_id
         FROM delegated_operations o JOIN missions m ON m.id=o.mission_id AND m.corp_id=o.corp_id
         WHERE o.corp_id=$1 AND o.actor_id=$2 AND o.idempotency_key=$3 FOR UPDATE OF o",
    )
    .bind(corp)
    .bind(actor)
    ...
    .map_err(db)?
    {
        if existing.get::<Uuid, _>("room_id") != room_id
            || existing.get::<String, _>("issuer") != issuer
            || existing.get::<String, _>("subject") != subject
        {
            return Err(ApiError::conflict(
                "Delegated request key already belongs to another request",
            ));
        }
        validate_job_binding(&mut tx, corp, actor, room_id, issuer, subject).await?;
        let ids = crony_store::MissionPlanIds {
            mission_id: existing.get("mission_id"),
            task_ids: vec![existing.get("task_id")],
        };
        tx.commit().await.map_err(db)?;
        return Ok((ids, Vec::new()));
    }
    let plan = plan.ok_or_else(|| ApiError::conflict("No connected delegated-resource runner"))?;
    ...
    validate_job_binding(&mut tx, corp, actor, room_id, issuer, subject).await?;
    let id = Uuid::new_v4();
    sqlx::query("INSERT INTO delegated_operations(id,corp_id,actor_id,mission_id,task_id,issuer,subject,status,expires_at,idempotency_key)
        VALUES($1,$2,$3,$4,$5,$6,$7,'waiting_for_authentication',now()+interval '15 minutes',$8)")
        .bind(id).bind(corp).bind(actor).bind(ids.mission_id).bind(ids.task_ids[0])
         .bind(issuer).bind(subject).bind(request.idempotency_key).execute(&mut *tx).await.map_err(db)?;
    tx.commit().await.map_err(db)?;
    Ok((ids, events))
}

async fn validate_job_binding(
    ...
    // Preserve admission's existing lock order. Replays use NOWAIT too, so an
    // authority change cannot deadlock or return a stale authorization result.
    let binding: Option<Uuid> = sqlx::query_scalar(
        "SELECT di.actor_id FROM delegated_identities di
         ...
         JOIN rooms room ON room.id=rm.room_id AND room.corp_id=di.corp_id
         ...
         FOR SHARE OF di,a,rm,room NOWAIT",
    )
    .bind(corp)
    .bind(actor)
    .bind(issuer)
