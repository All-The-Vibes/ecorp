...
fn assert_complete_archive(
    remote: &PublicationFixture,
    archive: &crony_audit::Archive,
    service: &Service,
) -> anyhow::Result<BTreeMap<String, Vec<u8>>> {
    use sha2::{Digest, Sha256};

    let checkpoint = archive.checkpoints.last().expect("fixture checkpoint");
    let retained = &service.witnesses[0];
    assert_eq!(archive.ledger_id, retained.ledger_id.to_string());
    archive.verify(
        &service.key.verifying_key(),
        Some(&retained.checkpoint_digest),
    )?;
    let sequence_path = format!(
        "audit/{}/{:020}",
        archive.ledger_id, checkpoint.checkpoint.last_sequence
    );
    let stem = format!("{sequence_path}-{}", checkpoint.digest);
    let archive_path = format!("{stem}/archive-00000.json");
    ...
    let index: crony_audit::ArchivePublicationIndex =
        serde_json::from_slice(files.get(&index_path).expect("complete archive index"))?;
    assert_eq!(index.schema_version, 1);
    assert_eq!(index.ledger_id, archive.ledger_id);
    assert_eq!(index.checkpoint_digest, checkpoint.digest);
    assert_eq!(index.byte_count, archive_bytes.len() as u64);
    assert_eq!(index.sha256, hex::encode(Sha256::digest(&archive_bytes)));
    assert_eq!(
        index.parts.len(),
        1,
        "the small fixture fits one archive part"
    );
    assert_eq!(index.parts[0].path, archive_path);
    assert_eq!(index.parts[0].byte_count, index.byte_count);
    assert_eq!(index.parts[0].sha256, index.sha256);
    let expected = BTreeMap::from([
        (
            format!("{sequence_path}.checkpoint"),
        ...
        (index_path, serde_json::to_vec(&index)?),
    ]);
    assert_eq!(
        files, expected,
        "publish the complete signed checkpoint and native archive"
    );
    assert_eq!(
        *remote.writes.lock().unwrap(),
        expected.len(),
        "create each immutable file once"
    );
    Ok(expected)
}
...
async fn b02_descendant_replay_bootstrap_and_database_fence(pool: PgPool) -> anyhow::Result<()> {
    ...
    let published = assert_complete_archive(
        &remote,
        &store.audit_export(ids.corp_id, ids.alice_actor_id).await?,
        &service,
    )?;
    assert!(
        store
    ...
    assert_eq!(*remote.files.lock().unwrap(), published);
    assert_eq!(*remote.writes.lock().unwrap(), published.len());
    ...
    let bootstrapped = assert_complete_archive(
        &bootstrap,
        &store.audit_export(ids.corp_id, ids.alice_actor_id).await?,
        &service,
    )?;
    ...
    assert_eq!(*bootstrap.files.lock().unwrap(), bootstrapped);
    assert_eq!(*bootstrap.writes.lock().unwrap(), bootstrapped.len());
    Ok(())
}
