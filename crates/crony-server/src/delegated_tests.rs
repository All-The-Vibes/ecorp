...
impl Fixture {
    ...
    async fn operation(&self) -> Operation {
        ...
        let (ids, _) = persist_job(
            ...
            &expected.issuer,
            &expected.subject,
            Some(&protected_plan(&capability)),
        )
        .await
        .unwrap();
        let (launch, _) = self
...
fn retry_plan() -> TaskGraphPlan {
    protected_plan(&RunnerCapability {
        name: "delegated-resource".into(),
        available: true,
        detail: None,
        models: vec![],
...
fn retry_request(f: &Fixture, key: Uuid) -> JobRequest {
    JobRequest {
        actor_id: f.ids.alice_actor_id,
...
async fn admission_counts(pool: &sqlx::PgPool) -> (i64, i64, i64, i64, i64, i64, i64) {
    sqlx::query_as(
        "SELECT (SELECT count(*) FROM missions),(SELECT count(*) FROM tasks),
        (SELECT count(*) FROM agents),(SELECT count(*) FROM actors),(SELECT count(*) FROM events),
        (SELECT count(*) FROM delegated_operations),(SELECT count(*) FROM runs)",
    )
    .fetch_one(pool)
    .await
    .unwrap()
...
#[sqlx::test(migrations = "../../db/migrations")]
#[ignore = "requires explicitly owned SQLx maintenance database"]
async fn delegated_job_concurrent_retry_and_disconnected_recovery(pool: sqlx::PgPool) {
    let f = Fixture::new(pool.clone()).await;
    let expected = f.provider.expected();
    let request = retry_request(&f, Uuid::new_v4());
    let plan = retry_plan();
    let before = admission_counts(&pool).await;
    ...
    let (first, first_events) = left.unwrap();
    let (second, second_events) = right.unwrap();
    assert_eq!(first.mission_id, second.mission_id);
    assert_eq!(first.task_ids, second.task_ids);
    assert_ne!(first_events.is_empty(), second_events.is_empty());
    let admitted = admission_counts(&pool).await;
    assert_eq!(
        admitted,
        (
            before.0 + 1,
            ...
            before.2 + 1,
            before.3 + 1,
            before.4 + (first_events.len() + second_events.len()) as i64,
            before.5 + 1,
            before.6
        )
    );
    // Exercise the actual HTTP handler with no connected runner, including the
    // commit-before-schedule recovery window. A new key still needs a runner.
    assert!(f.state.runners.is_empty());
    let answer = create_job(
        State(f.state.clone()),
        Extension(f.principal()),
        Path(f.ids.corp_id),
        Json(retry_request(&f, request.idempotency_key)),
    )
    .await
    .unwrap()
    .0;
    assert_eq!(
        answer,
        json!({"mission_id":first.mission_id,"task_id":first.task_ids[0]})
    );
    assert!(
        create_job(
            State(f.state.clone()),
            Extension(f.principal()),
            Path(f.ids.corp_id),
            Json(retry_request(&f, Uuid::new_v4()))
        )
        .await
        .is_err()
    );
    assert_eq!(admission_counts(&pool).await, admitted);
    let (launch, _) = f
        .state
        .store
        .create_task_run(
        ...
        .await
        .unwrap();
    sqlx::query("UPDATE delegated_operations SET run_id=$2 WHERE task_id=$1")
        .bind(first.task_ids[0])
        .bind(launch.run_id)
        .execute(&pool)
        .await
        .unwrap();
    let assigned = admission_counts(&pool).await;
    for _ in 0..2 {
        ...
        assert_eq!(admission_counts(&pool).await, assigned);
    }
    f.state
        .store
        .fail_run_before_dispatch(
            f.ids.corp_id,
            launch.run_id,
            "synthetic terminal retry control",
        )
        .await
        .unwrap();
    let terminal = admission_counts(&pool).await;
    assert_eq!(
        create_job(
            State(f.state.clone()),
            Extension(f.principal()),
    ...
    assert_eq!(admission_counts(&pool).await, terminal);
    let actual: (Uuid, Uuid, Uuid) = sqlx::query_as("SELECT mission_id,task_id,run_id FROM delegated_operations WHERE actor_id=$1 AND idempotency_key=$2")
        .bind(f.ids.alice_actor_id).bind(request.idempotency_key).fetch_one(&pool).await.unwrap();
    assert_eq!(actual, (first.mission_id, first.task_ids[0], launch.run_id));
    assert_eq!(f.resource_reads.load(Ordering::SeqCst), 0);
}

#[sqlx::test(migrations = "../../db/migrations")]
#[ignore = "requires explicitly owned SQLx maintenance database"]
async fn delegated_job_retry_rechecks_identity_room_and_role(pool: sqlx::PgPool) {
    let f = Fixture::new(pool.clone()).await;
    let expected = f.provider.expected();
    let request = retry_request(&f, Uuid::new_v4());
    persist_job(
    ...
    .await
    .unwrap();
    let baseline = admission_counts(&pool).await;
    let wrong_room = JobRequest {
        room_id: Uuid::new_v4(),
        ..retry_request(&f, request.idempotency_key)
    };
    assert!(
        persist_job(
            &f.state.store,
    ...
    for (issuer, subject) in [
        ("wrong-issuer", expected.subject.as_str()),
        (expected.issuer.as_str(), "wrong-subject"),
    ] {
        assert!(
            persist_job(
                &f.state.store,
    ...
    // A concurrently held role change must prevent replay from observing stale
    // authority; the request must fail without waiting for the transaction.
    let mut revocation = pool.begin().await.unwrap();
    sqlx::query("UPDATE actors SET role='viewer' WHERE id=$1")
        .bind(f.ids.alice_actor_id)
        .execute(&mut *revocation)
        .await
        .unwrap();
    ...
    sqlx::query("INSERT INTO room_memberships(room_id,actor_id) VALUES($1,$2)")
        ...
        .await
        .unwrap();
    sqlx::query("UPDATE delegated_identities SET subject='replacement-subject' WHERE actor_id=$1")
        .bind(f.ids.alice_actor_id)
        .execute(&pool)
        .await
        .unwrap();
    ...
    assert_eq!(admission_counts(&pool).await, baseline);
}

#[sqlx::test(migrations = "../../db/migrations")]
#[ignore = "requires explicitly owned SQLx maintenance database"]
async fn delegated_operation_foreign_keys_enforce_corp_and_exact_lineage(pool: sqlx::PgPool) {
    let f = Fixture::new(pool.clone()).await;
    let original = f.operation().await;
    let expected = f.provider.expected();
    let (other, _) = persist_job(
        &f.state.store,
        f.ids.corp_id,
        &retry_request(&f, Uuid::new_v4()),
        &expected.issuer,
        &expected.subject,
        Some(&retry_plan()),
    )
    ...
    sqlx::query("INSERT INTO corps(id,slug,name) VALUES($1,$2,'foreign-scope-fixture')")
        .bind(foreign_corp)
        .bind(foreign_corp.to_string())
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO actors(id,corp_id,name,kind,role) VALUES($1,$2,'foreign-actor','human','owner')")
        .bind(foreign_actor).bind(foreign_corp).execute(&pool).await.unwrap();
    for (query, value, constraint) in [
        (
            "UPDATE delegated_operations SET actor_id=$2 WHERE id=$1",
            foreign_actor,
            "delegated_operation_actor_scope",
        ),
        (
            "UPDATE delegated_operations SET mission_id=$2 WHERE id=$1",
            other.mission_id,
            "delegated_operation_task_scope",
        ),
        (
            "UPDATE delegated_operations SET run_id=$2 WHERE id=$1",
            other_run.run_id,
            "delegated_operation_run_scope",
        ),
    ] {
    ...
    let error = sqlx::query("UPDATE missions SET corp_id=$2 WHERE id=$1")
        .bind(original.mission)
        .bind(foreign_corp)
        .execute(&pool)
        .await
        .unwrap_err();
    ...
    let saved: (Uuid, Uuid, Uuid) =
        sqlx::query_as("SELECT corp_id,task_id,run_id FROM delegated_operations WHERE id=$1")
            .bind(original.id)
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(
        ...
        (
            f.ids.corp_id,
            original.assignment.task_id,
            original.assignment.run_id
        )
    );
}
