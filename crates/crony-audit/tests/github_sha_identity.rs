...
use crony_audit::{
    Archive, MAX_ARCHIVE_BYTES, MAX_RECORD_BYTES, SignedCheckpoint, TrustedSigningKey,
};
...
async fn native_github_sha_identity() {
    ...
    let generation_status = Command::new(openssl)
        .args([
            "req",
            "-x509",
            "-newkey",
    ...
    fs::write(
        ...
        format!("openssl_exit_code={:?}\n", generation_status.code()),
    )
    .unwrap();
    assert!(
        generation_status.success(),
        "TLS fixture generation exited with {:?}",
        generation_status.code()
    );
