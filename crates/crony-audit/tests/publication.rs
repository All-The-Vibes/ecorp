...
struct Fake {
    ...
    rejected_ancestor: Mutex<Option<String>>,
    ancestry_checks: Mutex<Vec<(String, String)>>,
}
    ...
    fn descends_from<'a>(&'a self, old: &'a str, new: &'a str) -> BoxFuture<'a, Result<bool>> {
        Box::pin(async move {
            ...
            Ok(!*self.rewritten.lock().unwrap()
                && self.rejected_ancestor.lock().unwrap().as_deref() != Some(old))
        })
    }
...
#[tokio::test]
async fn archive_fences_previous_and_retained_commits_before_any_remote_write() {
    let (archive, keys) = archive_fixture(2, 0);
    let checkpoint = archive.checkpoints.last().unwrap();
    for rejected in ["previous-destination", "independently-retained"] {
        let fake = Fake::default();
        *fake.rejected_ancestor.lock().unwrap() = Some(rejected.into());
        let error = publish_checkpoint_archive(
            ...
            Some("independently-retained"),
        )
        .await
        .unwrap_err();
        assert!(error.to_string().contains("history rewrite"));
        assert_eq!(*fake.head_calls.lock().unwrap(), 1);
        assert_eq!(*fake.writes.lock().unwrap(), 0);
        assert!(fake.files.lock().unwrap().is_empty());
        assert!(
            fake.ancestry_checks
                .lock()
                .unwrap()
                .contains(&(rejected.into(), "head-1".into()))
        );
    }

    let fake = Fake::default();
    let publication = publish_checkpoint_archive(
        &fake,
        "audit",
        checkpoint,
        &archive,
    ...
    .unwrap();
    assert_eq!(
        &fake.ancestry_checks.lock().unwrap()[..2],
        &[
            ("previous-destination".into(), "head-1".into()),
            ("independently-retained".into(), "head-1".into()),
        ],
    );
    assert_eq!(
        serde_json::to_vec(&reconstruct(&fake, &publication)).unwrap(),
...
async fn legacy_keyless_archive_requires_the_independent_historical_key() {
    ...
    let fake = Fake::default();
    let published =
        publish_checkpoint_archive(&fake, "audit", checkpoint, &archive, &keys, None, None)
            .await
            .unwrap();
...
async fn legacy_publication_extends_to_complete_archive_without_changing_v1() {
    ...
    let checkpoint = archive.checkpoints.last().unwrap();
    let fake = Fake::default();
    let legacy_head = publish_checkpoint(&fake, "audit", checkpoint, None, None)
        .await
        .unwrap();
...
async fn multipart_archive_is_deterministic_bounded_and_complete() {
    ...
    let second =
        publish_checkpoint_archive(&other, "audit", checkpoint, &archive, &keys, None, None)
            .await
            .unwrap();
...
async fn invalid_or_untrusted_archive_fails_before_remote_access() {
    ...
    for case in 0..9 {
        ...
        let fake = Fake::default();
        assert!(
            publish_checkpoint_archive(&fake, "audit", checkpoint, &invalid, &trusted, None, None)
                .await
                .is_err(),
            "case {case}"
        );
    ...
    for root in [
        ...
        let fake = Fake::default();
        assert!(
            publish_checkpoint_archive(&fake, &root, checkpoint, &archive, &keys, None, None)
                .await
                .is_err()
        );
        assert_eq!(*fake.head_calls.lock().unwrap(), 0);
...
async fn oversized_valid_archive_fails_before_writes() {
    ...
    let fake = Fake::default();
    let error = publish_checkpoint_archive(&fake, "audit", checkpoint, &archive, &keys, None, None)
        .await
        .unwrap_err();
    assert!(
        error
