...
async fn issue283_additive_guard_upgrades_existing_foundation(pool: PgPool) -> Result<()> {
    ...
    let foundation = sqlx::migrate::Migrator {
        migrations: std::borrow::Cow::Owned(
            source
                .iter()
                .filter(|m| m.version <= 46 || m.version == 51)
                .cloned()
                .collect(),
        ),
        ..sqlx::migrate::Migrator::DEFAULT
    ...
    assert_eq!(after[46].0, 51);
    assert!(matches!(
        previous.run(&pool).await,
        Err(sqlx::migrate::MigrateError::VersionMissing(51))
    ));
    Ok(())
}
...
async fn native_archive_publication_requires_complete_readback_and_upgrades_legacy_receipt(
    ...
    store
        ...
        .await?;
    let remote = PublicationFixture::default();
    crony_audit::publish_checkpoint(&remote, "audit", &checkpoint, None, None).await?;
...
async fn pr293_f01_publication_does_not_block_checkpoint_or_governance(pool: PgPool) -> Result<()> {
    ...
    // Keep this gate local: shared publication fixtures also count remote reads.
    struct GatedPublication {
        inner: PublicationFixture,
    ...
    impl crony_audit::PublicationTransport for GatedPublication {
        fn head(&self) -> futures_util::future::BoxFuture<'_, Result<String>> {
            Box::pin(async {
    ...
    let remote = std::sync::Arc::new(GatedPublication {
        inner: PublicationFixture::default(),
    ...
    let publisher = {
        ...
        tokio::spawn(async move {
            store
                .publish_audit_destination(destination.id, remote.as_ref(), &public_key, None)
                .await
        })
    };
    ...
    assert!(
        !timeout(
            ...
            store.publish_audit_destination(
                destination.id,
                remote.as_ref(),
                &key.verifying_key(),
                None
    ...
    assert_eq!(*remote.inner.writes.lock().unwrap(), 0);
