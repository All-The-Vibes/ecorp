...
$fixture = '<local-user>\code\qa\pr325-browser-fixture-20260922-r7'
$output = Join-Path $PSScriptRoot 'pr325-native-browser-r7'
$receiptPath = Join-Path $PSScriptRoot 'pr325-browser-receipt-r7.json'
...
$validation = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'pr325-validation-r11/validation.json') -Raw | ConvertFrom-Json
if ($validation.status -ne 'passed' -or $validation.staged_tree -ne $tree) { throw 'Current passing source-bound validation is required.' }
...
& node tools/e2e_verifier_browser.mjs *> (Join-Path $PSScriptRoot 'pr325-browser-r7.log')
