...
try {
  if (phase === 'prepare') {
    ...
    for (const nativeRun of settled.snapshot.runs) {
      ...
      await verifyDirectories()
      await writeFile(path.join(output, `${nativeRun.id}-provider-artifact.txt`), bytes, { flag: 'wx' })
    ...
    await verifyDirectories()
    await writeFile(path.join(output, 'native-final-snapshot.json'), JSON.stringify(settled, null, 2), { flag: 'wx' })
    assert.deepEqual(sourceIdentity(), checkpoint.source)
