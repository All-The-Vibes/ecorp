param(
  [Parameter(Mandatory)][ValidateSet(237,353,354,355,358,359)][int]$Number,
...
if ($FocusedCompletesBrowser -and ($Number -ne 359 -or [IO.Path]::GetExtension($FocusedScript) -ne '.mjs')) {
...
$qa = "<local-user>\code\qa\pr359-agent-pinning-20260922-$Revision"
...
Save-Receipt
$supervisor = Join-Path $PSScriptRoot 'qa-pr359-stack-r2.ps1'
$serverPort = 59031
$webPort = 59032
$databasePort = 59030
try {
  ...
  With-Cargo {
    ...
    if ($Number -eq 359) {$buildArgs+=@('-p','crony-cli')}
    & cargo @buildArgs *> $log
    ...
    if($Number -eq 359){$binaryNames+='crony-cli.exe'}
  ...
  if ($Number -eq 359) {
    ...
    $env:DATABASE_URL = "postgres://issue48:${databasePassword}@127.0.0.1:$databasePort/postgres"
    With-Cargo {
      ...
      $packages = @('-p','crony-store','-p','crony-server')
      $filter = 'issue48_'
      $log = Join-Path $PSScriptRoot "$prefix-sqlx.log"
      & cargo test @packages $filter -- --ignored --test-threads=1 2>&1 | ForEach-Object { ([string]$_).Replace($databasePassword,'[ephemeral database credential]') } | Set-Content -LiteralPath $log -Encoding utf8
