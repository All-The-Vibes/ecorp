...
[IO.File]::WriteAllText($pgpassPath,"127.0.0.1:${DatabasePort}:*:issue48:$databasePassword`n",[Text.UTF8Encoding]::new($false))
...
try {
    ...
    & (Join-Path $pg 'psql.exe') -X -w -h 127.0.0.1 -p $DatabasePort -U issue48 -d postgres -c 'SELECT 1' *> $null
    ...
    & (Join-Path $pg 'createdb.exe') -w -h 127.0.0.1 -p $DatabasePort -U issue48 issue48_app
...
$serverEnv = @{
    DATABASE_URL="postgres://issue48:${databasePassword}@127.0.0.1:$DatabasePort/issue48_app"
