...
async fn issue56_control_dispatch_revalidates_lease_after_run_wait_renew(pool: PgPool) {
    ...
    .enumerate()
    {
        ...
        let dispatch = tokio::spawn(async move {
            dispatcher
                .retrospective_progress_dispatch(&command, |_| {
...
#[sqlx::test(migrations = "../../db/migrations")]
#[ignore = "requires explicitly owned retrospective PostgreSQL"]
async fn issue56_control_dispatch_revalidates_lease_after_run_wait_release_reacquire(pool: PgPool) {
    let store = fixture(pool).await;
    let recipient = Uuid::new_v4();
    sqlx::query(
        "INSERT INTO actors(id,corp_id,name,kind,role) VALUES($1,$2,'Recipient','human','member')",
    ...
    .await
    .unwrap();
    for (offset, mutation) in ["release_reacquire"]
    .into_iter()
    .enumerate()
    {
        let index = 10 + offset as u128;
