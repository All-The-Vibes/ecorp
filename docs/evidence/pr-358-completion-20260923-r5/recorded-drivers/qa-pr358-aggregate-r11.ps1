...
if($env:ECORP_COMPLETION_PR -ne '358' -or
   ...
   (Split-Path -Leaf $qa) -notmatch '^pr265-run-activity-pr358-20260923-r11$'){
...
if(!$state.test_owned -or $state.plan.product -cne $product -or
   ([uri]$state.plan.server).Host -ne '127.0.0.1' -or ([uri]$state.plan.server).Port -lt 10000 -or
