...
<#
Owned Windows supervisor for PR358 aggregate and browser acceptance.
...
if ($Phase -in @('Status','Stop')) {
    ...
    if (Test-LocalOwnedProcess -Record $state.processes.postgres -Workspace $qa) {
        if (!(Stop-LocalOwnedProcess -Record $state.processes.postgres -Workspace $qa)) { throw 'Owned PostgreSQL handle shutdown failed; retained.' }
    }
...
$serverExe = Join-Path $product 'target-acceptance-r10\debug\crony-server.exe'
$runnerExe = Join-Path $product 'target-acceptance-r10\debug\crony-runner.exe'
