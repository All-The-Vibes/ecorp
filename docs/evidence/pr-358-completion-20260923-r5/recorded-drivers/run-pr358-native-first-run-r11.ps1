...
$FocusedScript = Join-Path $PSScriptRoot 'qa-pr358-aggregate-r11.ps1'
...
Save-Receipt
$supervisor = Join-Path $PSScriptRoot 'qa-pr358-stack-r10.ps1'
...
try {
  ...
  # Reuse only the exact previously built bytes and passed PostgreSQL suites.
  # The failed r10 acceptance never passed its external fixture-date guard.
  $priorPath=Join-Path $OutputDirectory 'pr358-native-r10-lifecycle.json'
  $prior=Get-Content -LiteralPath $priorPath -Raw|ConvertFrom-Json
  if($prior.tested_staged_tree -cne $tree -or $prior.validation_source_tree -cne $validationTree){throw 'Retained native evidence belongs to different source.'}
  $native=Join-Path $product 'target-acceptance-r10/debug'
  $receipt.binaries=$prior.binaries
  foreach($binary in $receipt.binaries){
    if((Get-FileHash -LiteralPath (Join-Path $native $binary.file)).Hash.ToLowerInvariant() -cne $binary.sha256){throw 'Retained native binary changed.'}
  }
  $required=@('native-build','owned-postgresql-retained-receipts-regressions','owned-postgresql-crony-store-regressions','owned-postgresql-crony-server-regressions')
  $bound=@($prior.checks|Where-Object name -in $required)
  if($bound.Count -ne 4 -or @($bound|Where-Object exit_code -ne 0).Count){throw 'Incomplete prior native coverage.'}
  foreach($check in $bound){
    if((Get-FileHash -LiteralPath $check.log).Hash.ToLowerInvariant() -cne $check.sha256){throw 'Retained native check evidence changed.'}
  }
  $receipt.retained_native_evidence=@{receipt=$priorPath;sha256=(Get-FileHash -LiteralPath $priorPath).Hash.ToLowerInvariant();checks=$bound;native_postgres_count=48;qualification='Reused unchanged source/binaries and all 48 already passed native PostgreSQL regressions. This r11 driver freshly executes aggregate, approval and browser acceptance.'}
  Save-Receipt
...
'@
  ...
  $state.processes.runner = Start-LocalOwnedProcess -Role 'runner-verifier-fixture' -Workspace $qa -FilePath (Join-Path $product 'target-acceptance-r10/debug/crony-runner.exe') -ArgumentList @() -WorkingDirectory $product -LogDirectory (Join-Path $qa 'logs') -Environment $runnerEnvironment
  ...
  $approvalReport = Join-Path $qa 'evidence/approval-execution.json'
  ...
  & node (Join-Path $PSScriptRoot 'e2e-approvals-pr358-r11.mjs') *> $log
