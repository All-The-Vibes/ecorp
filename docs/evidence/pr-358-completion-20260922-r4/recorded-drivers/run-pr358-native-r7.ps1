...
$FocusedScript = '' # Source-bound aggregate acceptance is explicitly reused from r6.
...
foreach ($dependency in @($supervisor)) { if (!(Test-Path -LiteralPath $dependency -PathType Leaf)) { throw 'Retained driver dependency is missing.' } }
try {
  ...
  $priorPath = Join-Path $PSScriptRoot 'pr358-native-r6-lifecycle.json'
  ...
  $requiredChecks = @('workspace-cache-refresh','native-build','sqlx-workspace-cache-refresh','owned-postgresql-retained-receipts-regressions','owned-postgresql-crony-store-regressions','owned-postgresql-crony-server-regressions','focused-native-acceptance')
