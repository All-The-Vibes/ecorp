...
$FocusedScript = Join-Path $PSScriptRoot 'qa-pr354-readiness-r7.mjs'
...
if ($validationTree -cne $tree) {
    ...
    if ($packet -notmatch '^docs/evidence/pr-354-[a-z0-9-]+$') { throw 'Different source tree without the exact published evidence packet.' }
...
Save-Receipt
$supervisor = Join-Path $PSScriptRoot 'qa-pr354-stack-r3.ps1'
...
try {
  ...
  $priorPath = Join-Path $PSScriptRoot 'pr354-native-r3-lifecycle.json'
  $prior = Get-Content -LiteralPath $priorPath -Raw | ConvertFrom-Json
  if ($prior.pr -ne $Number -or $prior.tested_staged_tree -cne $tree) { throw 'Reuse requires identical complete source tree.' }
  $requiredChecks = @('workspace-cache-refresh','native-build','sqlx-workspace-cache-refresh','owned-postgresql-factory-connection-regressions','owned-postgresql-dispatch-readiness-regressions')
  $selected = @($prior.checks | Where-Object { $_.name -in $requiredChecks })
  if ($selected.Count -ne $requiredChecks.Count -or (Compare-Object @($selected.name | Sort-Object) @($requiredChecks | Sort-Object))) { throw 'Missing prior verification.' }
  foreach ($check in $selected) {
    if ($check.exit_code -ne 0 -or (Get-FileHash -LiteralPath $check.log).Hash.ToLowerInvariant() -cne $check.sha256) { throw 'Prior verification bytes changed.' }
  }
  foreach ($binary in $prior.binaries) {
    if ($binary.file -cnotin @('crony-server.exe','crony-runner.exe','crony-cli.exe')) { throw 'Unexpected prior binary.' }
    if ((Get-FileHash -LiteralPath (Join-Path $product "target/debug/$($binary.file)")).Hash.ToLowerInvariant() -cne $binary.sha256) { throw 'Prior binary changed.' }
  }
  $receipt.binaries = $prior.binaries
  $receipt.reused_verification = @{
    receipt=$priorPath; receipt_sha256=(Get-FileHash -LiteralPath $priorPath).Hash.ToLowerInvariant()
    checks=$selected; source_tree=$tree
    reason='Identical complete source tree and verified original log and binary hashes. Named passing checks are reused explicitly, not represented as rerun. A fresh owned fixture runs the remaining acceptance.'
  }
  Save-Receipt
  ...
  foreach ($name in @([Environment]::GetEnvironmentVariables('Process').Keys)) {
    if ($name -match '^(PG|GH_|GITHUB_|AZURE_)' -or $name -in @('DATABASE_URL','OPENAI_API_KEY','ANTHROPIC_API_KEY','COPILOT_GITHUB_TOKEN')) {
      Remove-Item -LiteralPath ("Env:" + $name) -ErrorAction SilentlyContinue
    }
  }
  if ($FocusedScript) {
...
'@
  ...
  & node (Join-Path $PSScriptRoot 'qa-pr354-browser-preflight-r3.mjs') *> $log
