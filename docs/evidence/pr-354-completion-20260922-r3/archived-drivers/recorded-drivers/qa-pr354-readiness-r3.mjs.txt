...
try {
  ...
  proxy = createServer(async (incoming, outgoing) => {
    ...
    try {
      ...
      outgoing.end(result);
    } catch (error) { outgoing.writeHead(502); outgoing.end(String(error)); }
  });
  ...
  assert.equal(dryPreflight.body.require_dispatch_ready,false);
  ...
  assert.deepEqual(JSON.parse(await readFile(fakeStatePath,'utf8')),fakeState);
  assert.equal(await git(['-C',path.join(root,'source'),'rev-parse','HEAD']),sourceBefore);
