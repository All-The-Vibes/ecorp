...
function ledger() {
  ...
  return redactEvidenceCapabilities(JSON.parse(sql(`SELECT jsonb_build_object(${fields.join(', ')});`)));
...
try {
  ...
  report.ledger={capability_redaction:'SHA-256 before serialization and equality hashing; nulls retained',tables:Object.keys(withLegacy),before_sha256:digest(withLegacy),after_sha256:digest(ledger()),exact_rows_unchanged:true};
