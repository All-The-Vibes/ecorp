...
$build=Join-Path $PSScriptRoot 'pr305-native-r1-build'
$output=Join-Path $PSScriptRoot 'pr305-native-r1'
if((Test-Path -LiteralPath $build) -or (Test-Path -LiteralPath $output)){throw 'Preserve prior attempts.'}
$validation=Get-Content -LiteralPath (Join-Path $PSScriptRoot 'pr305-validation-r1/validation.json') -Raw|ConvertFrom-Json
...
try {
  ...
  & node tools/research_handoff_native.mjs (Join-Path $PSScriptRoot 'pr305-native-r1-manifest.json') $output *> (Join-Path $PSScriptRoot 'pr305-native-r1-driver.log')
  $code=$LASTEXITCODE
