param(
  [Parameter(Mandatory)][ValidateSet(294)][int]$Number,
...
if ($FocusedCompletesBrowser -and ([IO.Path]::GetExtension($FocusedScript) -ne '.ps1')) {
...
if ($validation.status -ne 'passed' -or $testedTree -cne $tree -or @($validation.checks).Count -ne 9 -or @($validation.checks | Where-Object exit_code -ne 0).Count -or (& git -C $product diff --name-only)) { throw 'Source must match nine passing validation gates.' }
...
Save-Receipt
$supervisor = Join-Path $PSScriptRoot 'qa-pr294-stack-r1.ps1'
$serverPort = 59014
$webPort = 26000 + $Number
$databasePort = 25000 + $Number
try {
  Set-Location -LiteralPath $product
  # CSS-only follow-up: verify backend bytes and binary hashes against the retained passing native evidence.
  $priorPath = Join-Path $PSScriptRoot 'pr294-native-r2-lifecycle.json'
  $prior = Get-Content -LiteralPath $priorPath -Raw | ConvertFrom-Json
  if ($prior.pr -ne 294 -or $prior.tested_staged_tree -cne '29c59aa86469db2e21880c140fc1ca119cc3276b' -or $prior.source_head -cne 'be187e5072223e3bd7dbc147293bfc18ecdac822') { throw 'Unexpected original native source.' }
  $allowed = @('apps/web/src/OperationsUx.css', 'docs/evidence/pr-294-completion-20260922/')
  $changed = @(& git -C $product diff --name-only $prior.tested_staged_tree $tree)
  if ($LASTEXITCODE -or !$changed.Count -or @($changed | Where-Object { $_ -cne $allowed[0] -and !$_.StartsWith($allowed[1],[StringComparison]::Ordinal) }).Count) { throw 'Native reuse requires unchanged backend and only the reviewed CSS/evidence delta.' }
  $browserPriorPath = Join-Path $PSScriptRoot 'pr294-native-r3-lifecycle.json'
  $browserPrior = Get-Content -LiteralPath $browserPriorPath -Raw | ConvertFrom-Json
  if ($browserPrior.status -cne 'passed' -or $browserPrior.tested_staged_tree -cne $prior.tested_staged_tree -or $browserPrior.reused_native_validation.sha256 -cne (Get-FileHash -LiteralPath $priorPath).Hash.ToLowerInvariant()) { throw 'Prior successful browser/native chain changed.' }
  $receipt.source_reuse = @{original_tree=$prior.tested_staged_tree;current_tree=$tree;changed_paths=$changed;backend_unchanged=$true;browser_prior=$browserPriorPath;browser_prior_sha256=(Get-FileHash -LiteralPath $browserPriorPath).Hash.ToLowerInvariant()}

  if ($prior.failure -cne 'browser-server-runner-policy-acceptance failed; preserve logs and fixture.') { throw 'Unexpected prior failure; do not reuse.' }
  $required = @('workspace-cache-refresh','native-build','sqlx-workspace-cache-refresh','owned-postgresql-crony-server-regressions')
  foreach ($name in $required) {
    $checks = @($prior.checks | Where-Object { $_.name -ceq $name })
    if ($checks.Count -ne 1 -or $checks[0].exit_code -ne 0) { throw 'Missing prior passing native check.' }
    if ((Get-FileHash -LiteralPath $checks[0].log).Hash.ToLowerInvariant() -cne $checks[0].sha256) { throw 'Prior log changed.' }
  }
  if (@($prior.binaries).Count -ne 2) { throw 'Expected server and runner receipts.' }
  foreach ($binary in $prior.binaries) {
    if ($binary.file -cnotin @('crony-server.exe','crony-runner.exe')) { throw 'Unexpected executable.' }
    $binaryPath = Join-Path $product "target/debug/$($binary.file)"
    if ((Get-FileHash -LiteralPath $binaryPath).Hash.ToLowerInvariant() -cne $binary.sha256) { throw 'Prior executable changed.' }
  }
  $receipt.binaries = $prior.binaries
  $receipt.reused_native_validation = @{lifecycle=$priorPath;sha256=(Get-FileHash -LiteralPath $priorPath).Hash.ToLowerInvariant();checks=@($prior.checks | Where-Object {$_.name -cin $required});reason='Backend source is unchanged; reviewed delta is OperationsUx.css plus the original immutable evidence packet. Reuse exact executable hashes and passing SQLx evidence; run a fresh real browser with contrast assertions.'}
  Save-Receipt
...
'@
  ...
  & node (Join-Path $PSScriptRoot 'qa-pr294-cache-browser-r4.mjs') *> $log
