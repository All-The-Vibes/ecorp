...
fixture = base / 'pr362-exact-staged-bytes-r4'
receipt_path = base / 'pr362-exact-staged-bytes-r4.json'
log = base / 'pr362-exact-staged-bytes-r4.log'
...
validation = json.loads((base/'pr362-validation-r4/validation.json').read_bytes())
...
receipt = {'pr': 362, 'tested_staged_tree': tree,
           ...
           'scope': 'Fourteen verifier regressions on exact staged Git bytes; gauntlet archive/inventory hashes and published path checks. No native-stack rerun.',
...
assert 'Ran 14 tests' in log.read_text(encoding='utf-8')
