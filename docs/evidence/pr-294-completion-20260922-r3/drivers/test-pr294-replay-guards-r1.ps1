...
$product='C:\Users\shyamsridhar\code\ecorp-pr294-completion-20260922'
$root=Join-Path $PSScriptRoot 'pr294-replay-guards-r1'
if(Test-Path -LiteralPath $root){throw 'Preserve guard-test evidence.'}
New-Item -ItemType Directory -Path $root|Out-Null
$driver=Join-Path $product 'docs/evidence/pr-294-completion-20260922-r3/drivers/run-pr294-native-r5.ps1'
$existingQa='C:\Users\shyamsridhar\code\qa\pr265-run-activity-pr294-20260922-r5'
...
$source=Get-Content -Raw -LiteralPath (Join-Path $product 'docs/evidence/pr-294-completion-20260922-r2/validation.json')|ConvertFrom-Json -AsHashtable
$cases=@(
...
foreach($case in $cases){
  ...
  if(Test-Path -LiteralPath (Join-Path $directory 'pr294-native-r999-lifecycle.json')){throw 'Guard test crossed the fixture mutation boundary.'}
  $outcomes+=@{name=$case.name;status='passed';observed_guard=$caught;fixture_mutations=$false}
}
if((Get-FileHash -LiteralPath $ownership).Hash -cne $before -or (& git -C $product write-tree).Trim() -cne $tree -or (& git -C $product status --porcelain=v1)){throw 'Guard tests changed retained fixture or product.'}
