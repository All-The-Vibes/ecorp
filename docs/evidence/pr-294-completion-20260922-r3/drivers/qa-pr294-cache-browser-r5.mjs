...
assert.ok(path.isAbsolute(process.env.ECORP_COMPLETION_PRODUCT ?? ''), 'Absolute reviewed product checkout required')
const { captureOwnedTestServerManifest } = await import(pathToFileURL(path.join(process.env.ECORP_COMPLETION_PRODUCT, 'tools/owned_test_stack.mjs')).href)
...
async function savedContractContrast(contract) {
  const rows = await contract.evaluate((root) => {
    const rgba = (value) => {
      if (!/^rgba?\(/.test(value)) throw new Error(`Unsupported measured color ${value}`)
    ...
    for (let node = walker.nextNode(); node; node = walker.nextNode()) {
      ...
      for (let parent = element; parent; parent = parent.parentElement) {
        ...
        if (style.backgroundImage !== 'none') throw new Error('Measure a flat saved policy surface')
        if (Number(style.opacity) !== 1) throw new Error('Unexpected translucent saved policy text')
      ...
      result.push({ text: text.slice(0, 180), color: style.color, background,
        font_size: style.fontSize, ratio: (Math.max(light, dark) + 0.05) / (Math.min(light, dark) + 0.05) })
    }
    return result
  })
  assert.ok(rows.length >= 10, 'Measure the actual expanded saved task contract')
  for (const row of rows) assert.ok(row.ratio >= 4.5, `Saved contract contrast ${row.ratio}: ${row.text}`)
  return { minimum_ratio: Math.min(...rows.map(row => row.ratio)), threshold: 4.5, rows }
}

async function explicitCachePolicy() {
  report.stage = 'explicit cache policy persisted through API and launched from browser'
  await save()
  const checks = ['command', 'test'].map((type) => ({ type, program: 'node',
    args: ['-e', "require('node:assert/strict').equal(process.env.NODE_DISABLE_COMPILE_CACHE, '1')"],
    timeout_ms: 10000, cache_suppression: 'node_compile_cache' }))
  const policy = { checks, manual_gate: null }
  const authored = { ...report.missions[0].authored,
    title: `[verification-matrix] Explicit node cache policy ${randomUUID().slice(0,8)}`,
    verification_policy: policy }
  const response = await fetch(`${server}/api/corps/${demo.corp_id}/missions`, {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(authored),
    redirect: 'error', signal: AbortSignal.timeout(15000) })
  assert.equal(response.status, 200)
  const created = await response.json()
  const saved = missionView(await snapshot(), created.mission_id)
  assert.equal(saved.mission.status, 'ready')
  assert.deepEqual(saved.tasks[0].verification_policy, policy)
  assert.equal(saved.runs.length, 0)
  const receipt = { mission_id: created.mission_id, authored, policy,
    setup: 'Supported HTTP create API; actual browser review and launch; no response mocks or database seeding',
    viewports: [] }
  report.explicit_cache_policy = receipt
  await page.goto(`${web}/#missions`, { waitUntil: 'networkidle' })
  await page.getByLabel('Work item', { exact: true }).selectOption(created.mission_id)
  const card = page.locator(`[data-mission-id="${created.mission_id}"]`)
  await card.waitFor()
  for (const [name, width, height] of [['desktop',1440,1050], ['mobile',390,844]]) {
    await page.setViewportSize({width,height})
    // These are the actual enclosing details in App.tsx. Use their visible summaries.
    const graph = card.locator('details.mission-task-dossier')
    assert.equal(await graph.count(), 1)
    if (!(await graph.evaluate((element) => element.open))) await graph.locator(':scope > summary').click()
    const task = graph.locator(`details.task-graph-item[data-task-id="${saved.tasks[0].id}"]`)
    assert.equal(await task.count(), 1)
    if (!(await task.evaluate((element) => element.open))) await task.locator(':scope > summary').click()
    const preview = task.getByTestId('verification-policy-preview')
    await preview.waitFor({state:'visible'})
    const text = await preview.innerText()
    assert.equal((text.match(/Requested cache control: Node compile cache \(requires a compatible runner\)/g) || []).length,2)
    receipt.viewports.push({name,width,height,text,contrast:await savedContractContrast(task.locator(".task-contract")),layout:await noOverflow(width),screenshot:await screenshot(`cache-${name}-ready`)})
  }
  const launchResponse = page.waitForResponse((value)=>value.url()===`${server}/api/corps/${demo.corp_id}/missions/${created.mission_id}/launch` && value.request().method()==='POST')
  await card.getByTestId('work-result-card').getByRole('button',{name:'Start mission',exact:true}).click()
  assert.equal((await launchResponse).status(),200)
  const final=await until('explicit cache verifier completion',async()=>missionView(await snapshot(),created.mission_id),
    (value)=>value.runs.length===1 && value.mission.status==='completed' && value.runs[0].workspace_disposition==='preserved')
  const run=final.runs[0]
  assert.equal(run.runner_id,setup.runner_id)
  assert.equal(run.status,'completed')
  assert.notEqual(path.resolve(run.workspace_path).toLowerCase(),path.resolve(setup.source).toLowerCase())
  ...
  for(const item of evidence){
    ...
    assert.deepEqual(item.payload.cache_suppression,{
      policy:'node_compile_cache',source:'explicit',scope:'verifier_child',
      requested_environment:{NODE_DISABLE_COMPILE_CACHE:'1'},requested_argument_prefix:[],
      zero_cache_writes_verified:false,cleanup_authorized:false})
  }
  assert.deepEqual(final.tasks[0].verification_policy,policy)
  receipt.final={mission:final.mission,tasks:final.tasks,run,evidence}
  await until('cache completion browser projection',()=>card.innerText(),(text)=>/completed/i.test(text),30000)
  receipt.screenshot=await screenshot('cache-completed')
  await save()
}
