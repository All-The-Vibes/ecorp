...
Exact committed-byte follow-up: all six verifier regressions were rerun from 38 files materialized with git show at 8c3e718563be568221b16d2ac8dfc9cb5ca74486. Each executed SHA-256 equals the committed Git blob SHA-256, and all 38 files remained unchanged. The committed manifest SHA-256 is f356809d422b4fe91e91485be4540e32d3164bc3e21f1d506843331f38edfe9e.

The prior Windows worktree manifest digest was eb6e997379fa79693beed3ee25d31e14226e87ef1c84d826c30ae247ee9eff7b; converting exactly 1553 CRLF endings to LF produces the committed bytes. That older worktree run is retained with its original binding and is not relabelled as an exact committed-byte run. The new receipt, full six-test log, report and recorded driver are retained here, and source-binding.json explicitly links this follow-up. Existing nine-gate logs remain evidence of their original run. No additional native stack run is claimed.

To reproduce the exact-byte check, materialize tools/test_public_evidence_verifier.py and all files in docs/evidence/pr362-startup-20260921, docs/evidence/pr362-regressions-20260921 and docs/evidence/pr362-combined-20260921 using git show from the recorded commit into a new owned directory. Run `python -X utf8 -m unittest discover -s tools -p test_public_evidence_verifier.py -v` there with PYTHONOPTIMIZE unset and PYTHONDONTWRITEBYTECODE=1. Hash the files before and after. The retained driver is the recorded structure and needs explicit new local paths; placeholders are not executable host configuration.
