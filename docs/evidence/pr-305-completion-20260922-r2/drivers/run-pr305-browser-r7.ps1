...
$qa='C:\Users\shyamsridhar\code\qa\pr265-run-activity-pr305-20260922-browser-r7'
$pg='C:\Users\shyamsridhar\AppData\Local\Programs\ecorp-tools\postgresql-17.10\pgsql\bin'
$prefix='pr305-browser-r7'
...
if($validation.status -ne 'passed' -or @($validation.checks).Count -ne 9 -or @($validation.checks|Where-Object exit_code -ne 0).Count -or $validation.staged_tree -cne $tree -or (& git -C $product diff --name-only)){throw 'Current exact staged source must match all nine passing gates.'}
$sourceDelta=@()
...
$receipt=[ordered]@{validation_scope='Fresh native/browser acceptance on the exact source tree that passed all nine gates.';prior_gate_tree=$validation.staged_tree;source_delta=$sourceDelta;pr=305;tested_staged_tree=$tree;head=(& git -C $product rev-parse HEAD).Trim();started_at_utc=[DateTimeOffset]::UtcNow.ToString('o');status='running';qa_root=$qa;checks=@();binaries=@()}
