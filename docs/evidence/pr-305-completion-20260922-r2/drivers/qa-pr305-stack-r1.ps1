...
<#
Owned Windows supervisor for PR305's exact-candidate static App acceptance.
...
$plan = [ordered]@{
    ...
    runner_id='30500000-0000-4000-8000-000000000001'; provider='native deterministic fake-process; no AI inference'
    source='independent clone of the exact candidate HEAD; writes execute only in runner worktrees'
...
& git -c core.longpaths=true clone --no-hardlinks --no-checkout $product $source
if ($LASTEXITCODE) { throw 'Independent candidate clone failed.' }
& git -C $source -c core.longpaths=true checkout --detach $plan.product_commit
if ($LASTEXITCODE) { throw 'Candidate fixture checkout failed.' }
& git -C $source remote set-url origin https://github.com/All-The-Vibes/ecorp.git
if ($LASTEXITCODE -or (& git -C $source rev-parse HEAD).Trim() -cne $plan.product_commit.Trim() -or (& git -C $source status --porcelain=v1)) { throw 'Candidate fixture identity differs.' }
$webBuild = Join-Path $qa 'built-web'
$priorVite = $env:VITE_CRONY_SERVER_HTTP
Push-Location -LiteralPath (Join-Path $product 'apps/web')
try {
    $env:VITE_CRONY_SERVER_HTTP = $plan.server
    & $node $vite build --outDir $webBuild *> (Join-Path $qa 'logs/web-build.log')
    if ($LASTEXITCODE) { throw 'Bound static App build failed.' }
} finally { Pop-Location; $env:VITE_CRONY_SERVER_HTTP = $priorVite }
...
Launch 'web' $node @($vite,'preview','--host','127.0.0.1','--port',"$WebPort",'--strictPort','--outDir',$webBuild) (Join-Path $product 'apps\web') @{
...
$state.demo=$demo
$state.source=@{repository='all-the-vibes/ecorp';base_ref='HEAD';base_commit=(& git -C $source rev-parse HEAD).Trim()}
