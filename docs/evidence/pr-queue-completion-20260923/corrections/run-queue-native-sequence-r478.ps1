...
$receiptPath=Join-Path $PSScriptRoot 'queue-native-sequence-r478.json'
...
function Step([string]$Name,[scriptblock]$Action){
 $log=Join-Path $PSScriptRoot "queue-native-sequence-r478-$Name.log"
...
try{
 Step 'verify-prior-suites' {
  ...
  foreach($relative in @('queue-audit-native-r471/acceptance-run.json','queue-delegated-native-r433.json','queue-native-database-r455.json')){
   ...
   $native=Get-Content -LiteralPath $path -Raw|ConvertFrom-Json
   $nativeTree=if($native.tested_staged_tree){$native.tested_staged_tree}else{$native.staged_tree}
   if($native.status -ne 'passed' -or $nativeTree -cne $v.staged_tree){throw 'Matching-source native acceptance required.'}
