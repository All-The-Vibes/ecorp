...
$run=Join-Path $PSScriptRoot 'queue-audit-native-r466'
...
$hostRoot=Join-Path $env:USERPROFILE '.copilot/session-state/ecorp-queue-audit-20260923-r466/files/native'
$output=Join-Path $repo 'output/native-qualification/phase2-queue-r466'
...
try{
  Step 'initialize-postgres' {
    ...
    $record=Start-LocalOwnedProcess -Role 'postgres-queue-audit-r466' -Workspace $run -FilePath (Join-Path $pg 'postgres.exe') -ArgumentList @('-D',$cluster,'-h','127.0.0.1','-p','55483','-c','timezone=UTC') -WorkingDirectory $run -LogDirectory (Join-Path $run 'postgres-logs') -Environment @{}
