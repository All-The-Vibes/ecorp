...
$receiptPath=Join-Path $PSScriptRoot 'queue-native-sequence-r471.json'
...
function Step([string]$Name,[scriptblock]$Action){
 $log=Join-Path $PSScriptRoot "queue-native-sequence-r471-$Name.log"
...
try{
 Step 'native-build' {
  ...
  $buildPath=Join-Path $PSScriptRoot 'queue-audit-native-r471/build-receipt.json'
 ...
 Step 'audit' { & pwsh -NoProfile -File (Join-Path $PSScriptRoot 'run-queue-audit-native-r471.ps1') }
 Step 'audit-owned-stop' {
  $cleanup=Get-Content -LiteralPath (Join-Path $PSScriptRoot 'queue-audit-native-r471/cleanup-r471.json') -Raw|ConvertFrom-Json
