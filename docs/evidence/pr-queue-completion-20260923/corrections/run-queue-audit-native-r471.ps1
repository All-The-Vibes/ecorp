...
$run=Join-Path $PSScriptRoot 'queue-audit-native-r471'
...
$hostRoot=Join-Path $env:USERPROFILE '.copilot/session-state/ecorp-queue-audit-20260923-r471/files/native'
$output=Join-Path $repo 'output/native-qualification/phase2-queue-r471'
...
function Stop-OwnedFixture {
  $cleanupPath=Join-Path $run 'cleanup-r471.json'
...
try{
  Step 'initialize-postgres' {
    ...
    $record=Start-LocalOwnedProcess -Role 'postgres-queue-audit-r471' -Workspace $run -FilePath (Join-Path $pg 'postgres.exe') -ArgumentList @('-D',$cluster,'-h','127.0.0.1','-p','55483','-c','timezone=UTC') -WorkingDirectory $run -LogDirectory (Join-Path $run 'postgres-logs') -Environment @{}
