...
$qa="C:\Users\shyamsridhar\code\qa\pr293-audit-store-20260923-$Revision"
...
$prefix="pr293-native-database-$Revision"
...
$receipt=[ordered]@{pr=293;purpose='Base V2 owned PostgreSQL regressions';source_head=(& git -C $product rev-parse HEAD).Trim();tested_staged_tree=$tree;qa_root=$qa;target=$target;started_at_utc=[DateTimeOffset]::UtcNow.ToString('o');status='running';checks=@();cleanup='pending'}
$state=@{schema_version=2;workspace=$qa;purpose='pr293-audit-store';test_owned=$true;processes=@{};plan=@{database=@{host='127.0.0.1';port=$port;user='audit293';name='postgres'}}}
...
try{
    ...
    foreach($check in @(
        @{name='base-v2';arguments=@('test','--locked','-p','crony-store','base_v2_','--','--ignored','--test-threads=1')},
        @{name='worker-observation';arguments=@('test','--locked','-p','crony-server','base_worker_observation_paging_http_restart_and_finalized_boundary','--','--ignored','--test-threads=1')},
        @{name='native-archive';arguments=@('test','--locked','-p','crony-store','-p','crony-server','state_audit','--','--ignored','--test-threads=1')}
    )){
