...
Import-Module (Join-Path 'C:\Users\shyamsridhar\code\ecorp-pr293-completion-20260922\tools' 'local_stack.psm1') -Force
$root = (Resolve-Path (Join-Path 'C:\Users\shyamsridhar\code\ecorp-pr293-completion-20260922\tools' '..')).Path
...
function Retain-AnvilChild {
    ...
    $expected = Join-Path 'C:\Users\shyamsridhar\code\ecorp-pr293-completion-20260922\tools' 'registry-toolchain\node_modules\@foundry-rs\anvil-win32-amd64\bin\anvil.exe'
...
if ($Action -eq 'start') {
    ...
    foreach ($name in @($databaseName,$journalName)) {
        $found = & 'C:\Users\shyamsridhar\AppData\Local\Programs\ecorp-tools\postgresql-17.10\pgsql\bin\psql.exe' -h 127.0.0.1 -p 55483 -U postgres -d postgres -Atc "SELECT 1 FROM pg_database WHERE datname='$name';"
        if ($LASTEXITCODE -ne 0) { throw 'Owned PostgreSQL unavailable.' }
        if ($found -ne '1') {
            & 'C:\Users\shyamsridhar\AppData\Local\Programs\ecorp-tools\postgresql-17.10\pgsql\bin\createdb.exe' -h 127.0.0.1 -p 55483 -U postgres $name *> $null
    ...
    Launch 'anvil' (Get-Command pwsh.exe).Source @('-NoProfile','-File',(Join-Path 'C:\Users\shyamsridhar\code\ecorp-pr293-completion-20260922\tools' 'start_base_registry_anvil.ps1'),'-Port','18557','-ChainId','84532','-CacheDirectory',(Join-Path $hostRoot 'anvil-cache')) $hostRoot @{}
    Wait-Ready 'anvil' {
    ...
    Retain-AnvilChild
    Launch 'github' $node @((Join-Path 'C:\Users\shyamsridhar\code\ecorp-pr293-completion-20260922\tools' 'native_qualification_github.mjs')) $hostRoot @{CRONY_NATIVE_QUALIFICATION='1';CRONY_NATIVE_OUTPUT=$output}
