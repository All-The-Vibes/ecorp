...
'@
    ...
    if ($databasePassword -notmatch '^[a-f0-9]{64}$') { throw 'Invalid owned database credential.' }
    $report.database_authentication = @{method='scram-sha-256';delivery='environment-only reduced assurance; credential file outside synthetic source and runner workspaces'}
    $configuration=@{
        DATABASE_URL="postgres://pr265_qa:${databasePassword}@127.0.0.1:$($qaState.plan.database.port)/pr265_activity";
