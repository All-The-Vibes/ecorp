...
try {
    ...
    & pwsh -NoLogo -NoProfile -NonInteractive -File (Join-Path $repo 'tools/local_stack_lifecycle.test.ps1') -Suite Startup -NodePath $receipt.node -FixtureParent $out -RetainFixtures *> $log
    $code=$LASTEXITCODE
