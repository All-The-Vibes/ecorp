...
$qa="<local-user>\code\qa\pr283-audit-store-20260922-$Revision"
...
$prefix="pr283-native-database-$Revision"
...
$receipt=[ordered]@{pr=283;purpose='audit SQLx regressions';source_head=(& git -C $product rev-parse HEAD).Trim();tested_staged_tree=$tree;qa_root=$qa;target=$target;started_at_utc=[DateTimeOffset]::UtcNow.ToString('o');status='running';checks=@();cleanup='pending'}
$state=@{schema_version=2;workspace=$qa;purpose='pr283-audit-store';test_owned=$true;processes=@{};plan=@{database=@{host='127.0.0.1';port=$port;user='audit283';name='postgres'}}}
...
try{
    ...
    [IO.File]::WriteAllText($passfile,"127.0.0.1:${port}:*:audit283:$secret`n",[Text.UTF8Encoding]::new($false))
    $log=Join-Path $PSScriptRoot "$prefix-initdb.log"
    & (Join-Path $pg 'initdb.exe') -D (Join-Path $qa 'database') -U audit283 --auth=scram-sha-256 --encoding=UTF8 --locale=C --pwfile=$passwordPath *> $log
    ...
    do{
        & (Join-Path $pg 'pg_isready.exe') -h 127.0.0.1 -p $port -U audit283 *> $null
    ...
    & (Join-Path $pg 'psql.exe') -X -w -h 127.0.0.1 -p $port -U audit283 -d postgres -c 'SELECT 1' *> $null
    ...
    $env:DATABASE_URL="postgres://audit283:${secret}@127.0.0.1:$port/postgres"
    $log=Join-Path $PSScriptRoot "$prefix-sqlx.log"
    & cargo test --locked -p crony-store -p crony-server state_audit -- --ignored --test-threads=1 2>&1 | ForEach-Object{([string]$_).Replace($secret,'[ephemeral database credential]')} | Set-Content -LiteralPath $log -Encoding utf8
