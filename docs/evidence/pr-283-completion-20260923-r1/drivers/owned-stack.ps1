...
<#
Owned Windows supervisor for PR283's deterministic browser acceptance.
...
if ($Phase -in @('Status','Stop')) {
    ...
    if (!$state -or $state.purpose -ne 'pr283-state-audit') { throw 'Missing exact fixture ownership.' }
    if ($Phase -eq 'Status') {
...
$serverExe = Join-Path $BinaryDirectory 'crony-server.exe'
$runnerExe = Join-Path $BinaryDirectory 'crony-runner.exe'
...
$plan = [ordered]@{
    ...
    database=@{host='127.0.0.1';port=$DatabasePort;name='audit283_app';fresh=$true}
    runner_id='runner-audit283-2830923'; provider='native deterministic fake-process; no AI inference'
...
[IO.File]::WriteAllText((Join-Path $source 'seed.txt'), "# PR283 deterministic source fixture`n")
...
& git -C $source remote add origin https://github.com/ecorp-fixture/pr283-state-audit.git
...
[IO.File]::WriteAllText($pgpassPath,"127.0.0.1:${DatabasePort}:*:audit283:$databasePassword`n",[Text.UTF8Encoding]::new($false))
& (Join-Path $pg 'initdb.exe') -D (Join-Path $qa 'database') -U audit283 --auth=scram-sha-256 --encoding=UTF8 --locale=C --pwfile=$passwordPath
if ($LASTEXITCODE) { throw 'Fresh PostgreSQL initialization failed; preserve this root.' }
$state = @{schema_version=2;workspace=$qa;purpose='pr283-state-audit';test_owned=$true;processes=@{};plan=$plan}
...
Wait-Ready { & (Join-Path $pg 'pg_isready.exe') -h 127.0.0.1 -p $DatabasePort -U audit283 *> $null; $LASTEXITCODE -eq 0 } 'PostgreSQL'
...
try {
    ...
    & (Join-Path $pg 'psql.exe') -X -w -h 127.0.0.1 -p $DatabasePort -U audit283 -d postgres -c 'SELECT 1' *> $null
    ...
    & (Join-Path $pg 'createdb.exe') -w -h 127.0.0.1 -p $DatabasePort -U audit283 audit283_app
...
& $node (Join-Path $PSScriptRoot 'generate-pr283-audit-key.mjs') $qa
if ($LASTEXITCODE) { throw 'Owned signing-key generation failed; preserve fixture.' }
$serverEnv = @{
    CRONY_STATE_AUDIT_SIGNING_KEY_FILE=(Join-Path $qa 'credentials/audit-seed.bin')
    CRONY_STATE_AUDIT_KEY_ID='pr283-owned-acceptance-20260923'
    CRONY_STATE_AUDIT_CHECKPOINT_SECONDS='3600'
    DATABASE_URL="postgres://audit283:${databasePassword}@127.0.0.1:$DatabasePort/audit283_app"
...
$state.demo=$demo
$state.source=@{repository='ecorp-fixture/pr283-state-audit';base_ref='HEAD';base_commit=(& git -C $source rev-parse HEAD)}
