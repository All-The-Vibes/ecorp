...
$root=Join-Path $PSScriptRoot 'pr355-retrospective-r1'
$qa='C:\Users\shyamsridhar\code\qa\pr355-retrospective-20260922-r1'
...
function Test-Case([string]$Repository,[string]$Name,[string]$Filter,[string]$Expected){
  ...
  $observed=switch($Expected){
    ...
    'behavioral-red' {$exitCode -ne 0 -and $text -match 'test result: FAILED\. 0 passed; 1 failed;' -and $text -match 'left: String\("ready"\)' -and $text -match 'right: "failed"' -and $text -match 'deliverable_failure_tests.rs:177:'}
  }
