...
<#
Owned Windows supervisor for PR265's deterministic browser acceptance.
...
try {
    ...
    if ($null -eq $priorPgpass) { Remove-Item -LiteralPath Env:PGPASSFILE -ErrorAction SilentlyContinue } else { $env:PGPASSFILE = $priorPgpass }
    if ($null -eq $priorPgpassword) { Remove-Item -LiteralPath Env:PGPASSWORD -ErrorAction SilentlyContinue } else { $env:PGPASSWORD = $priorPgpassword }
}
$serverEnv = @{
