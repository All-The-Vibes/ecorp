...
# This PR always runs focused preserved-deliverable acceptance AND the real browser lane.
param(
  [ValidateSet(355)][int]$Number = 355,
  ...
  [ValidateSet(29355)][int]$ServerPort = 29355,
  [ValidateSet(26355)][int]$WebPort = 26355,
  [ValidateSet(25355)][int]$DatabasePort = 25355
)
...
$FocusedScript = Join-Path $PSScriptRoot 'qa-pr355-deliverable-r6.mjs'
...
if ($validationTree -cne $tree) {
    ...
    if ($packet -notmatch '^docs/evidence/pr-355-[a-z0-9-]+$') { throw 'Different source tree without the exact published evidence packet.' }
...
Save-Receipt
$supervisor = Join-Path $PSScriptRoot 'qa-pr355-stack-r6.ps1'
...
try {
  ...
  if ($Number -eq 355) {
    ...
    With-Cargo {
      ...
      $suites = @(
        @{package='crony-store';filter='deliverable_failure_tests::';name='deliverable-failure';expected=3}
      )
...
'@
  ...
  & node (Join-Path $PSScriptRoot 'qa-pr355-browser-preflight-r6.mjs') *> $log
