...
try {
  ...
  # Only private browser navigation changed. Reuse the exact, verified r2 binaries
  # and passing SQLx receipt without repeating already-passing source checks.
  ...
  if ($prior.pr -ne 294 -or $prior.tested_staged_tree -cne $tree -or $prior.source_head -cne $receipt.source_head) { throw 'Reused native source identity differs.' }
  ...
  $receipt.reused_native_validation = @{lifecycle=$priorPath;sha256=(Get-FileHash -LiteralPath $priorPath).Hash.ToLowerInvariant();checks=@($prior.checks | Where-Object {$_.name -cin $required});reason='Same passing staged source and exact executable hashes; only private browser navigation changed.'}
  Save-Receipt
...
'@
  ...
  & node (Join-Path $PSScriptRoot 'qa-pr294-cache-browser-r3.mjs') *> $log
