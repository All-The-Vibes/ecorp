param(
  [Parameter(Mandatory)][ValidateSet(358)][int]$Number,
...
Save-Receipt
$supervisor = Join-Path $PSScriptRoot 'qa-pr358-stack-r1.ps1'
$serverPort = 59018
  ...
  if ($Number -eq 358) {
    ...
    With-Cargo {
      ...
      foreach ($package in @('crony-store','crony-server')) {
        $log = Join-Path $PSScriptRoot "$prefix-sqlx-$package.log"
        & cargo test -p $package issue56_ -- --ignored --test-threads=1 2>&1 | ForEach-Object { ([string]$_).Replace($databasePassword,'[ephemeral database credential]') } | Set-Content -LiteralPath $log -Encoding utf8
...
'@
  ...
  $log = Join-Path $PSScriptRoot "$prefix-approval-expiry.log"
