...
The retrospective baseline ran the recorded contemporaneous 16-case variant against the prior implementation: eight passed and eight failed, including four observed outside-byte reads. The receipt identifies test SHA-256 `caee91c1dfca31da8372eb10c22274f2a57cc17be2bf0833c4c99db83b804427`; those exact test bytes are unavailable. Byte identity with the final Git LF tests (`ba7a482f…`) or their CRLF checkout (`167496b6…`) is unverified. References to “current” tests in the retained receipt, provenance and archived driver describe historical orchestration, not established final-test identity or original development chronology. Earlier failed attempts remain under prior-attempts; those outcomes are not rewritten. Windows file identity is bound before reading, with ctime tracked after descriptor acquisition because NTFS may finalize a fresh file's change time at open. This tooling result is not new issue #225 native Factory acceptance.

[`baseline-clarification.png`](baseline-clarification.png) is a genuine browser screenshot of [`baseline-clarification.html`](baseline-clarification.html), newly rendered from the saved baseline receipt/log and separately identified candidate results. It is a saved-results report, not a test rerun, original terminal capture or application screen. The report preserves the baseline's exit 1 and distinguishes its recorded test variant from the final tests; no historical artifact is replaced.
