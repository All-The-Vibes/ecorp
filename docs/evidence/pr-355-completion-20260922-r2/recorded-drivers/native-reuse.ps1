param(
  [Parameter(Mandatory)][ValidateSet(355)][int]$Number,
...
Save-Receipt
$supervisor = Join-Path $PSScriptRoot 'qa-pr355-stack-r5.ps1'
...
try {
  ...
  $permitted = @('apps/web/src/App.tsx','apps/web/src/missionReviewSummary.test.mjs','docs/multiplayer/WORKING_SESSION.md')
  ...
  $receipt.reused_verification = @{receipt=$ReuseReceipt;receipt_sha256=(Get-FileHash -LiteralPath $ReuseReceipt).Hash.ToLowerInvariant();checks=$selected;reason='All native build inputs are identical to the original source tree and binary hashes match. The complete source comparison permits only explicitly reviewed current-main web UI and its regression, working-session documentation, and evidence documents. New acceptance runs on the current source; the original native build is reused explicitly.'}
  Save-Receipt
