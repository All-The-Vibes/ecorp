...
// The renderer receives only native browser startup variables. In particular,
// no fixture database/runner credentials or credential-file locators are inherited.
const browserEnvironment = Object.fromEntries([
  'SystemRoot', 'WINDIR', 'TEMP', 'TMP', 'USERPROFILE', 'LOCALAPPDATA', 'APPDATA',
  'PATH', 'PATHEXT', 'COMSPEC', 'ProgramFiles', 'ProgramFiles(x86)', 'ProgramW6432',
].filter(name => process.env[name] !== undefined).map(name => [name, process.env[name]]))
const browser = await chromium.launch({ channel: 'msedge', headless: true, env: browserEnvironment })
const receipt = { schema: 1, kind: 'Live browser command runner; direct child-process output, not replayed logs or an application screen', command, source_head: git('rev-parse', 'HEAD'), tested_staged_tree: stagedTree, browser: browser.version(), browser_environment_names: Object.keys(browserEnvironment).sort(), captures: [] }
let child
let exitCode = null
let exitSignal = null
...
try {
  ...
  const script = `& ${quote(program)} ${args.map(quote).join(' ')}; exit $LASTEXITCODE`
