...
The retained reproduce-native.ps1 is a normalized record of the private supervisor. It requires explicit replacement of local path placeholders, a fresh owned QA root, the retained reproduce-readiness.mjs, and a new passing validation receipt for the source being reproduced; it is not runnable unchanged from this packet. The corrected receipt reader accepts the public tested_staged_tree and private staged_tree schemas and permits only subsequent evidence documentation to differ. The original recorded script and its hash remain under prior-attempts/reproduce-native-before-receipt-fix.ps1; source-binding.json labels this correction separately from the historical run. The repository supervisor is tools/qa_factory_run_activity.ps1 and the browser entrypoint is tools/e2e_verification_policy_browser.mjs. Build the exact server and runner first and run cargo test -p crony-server issue256_ -- --ignored --test-threads=1 against isolated PostgreSQL. Keep external providers and publication disabled.
