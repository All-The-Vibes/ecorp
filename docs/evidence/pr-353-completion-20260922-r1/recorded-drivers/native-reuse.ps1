...
try {
  ...
  if ($prior.pr -ne $Number -or $prior.tested_staged_tree -ne $tree) { throw 'Prior build is for different source.' }
  ...
  $receipt.reused_verification = @{receipt=$ReuseReceipt;receipt_sha256=(Get-FileHash -LiteralPath $ReuseReceipt).Hash.ToLowerInvariant();checks=$selected;reason='Source and binary hashes match. Successful build/database checks are reused explicitly; only the corrected private acceptance driver runs again on a new owned fixture.'}
  Save-Receipt
