...
const worktree = path.resolve(process.env.ECORP_ISSUE161_REPOSITORY)
...
function context(name, port) {
  return {
    ...
    // Disposable fixture credential, environment delivery to the server is reduced assurance.
    databaseUrl: `postgres://ecorp_qa161:${encodeURIComponent(readFileSync(path.join(qa, 'secrets/pg-password.txt'), 'utf8').trim())}@127.0.0.1:55461/issue161_${name}`,
