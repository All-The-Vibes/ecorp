...
function ScopedDelegatedPanel({ corpId, roomId, actorId, api, serverUrl, onRefresh }: DelegatedPanelProps) {
  ...
  const pendingJobStorageKey = `ecorp:delegated-job:${JSON.stringify([serverUrl, corpId, actorId, roomId])}`
  const pendingJobKey = useRef<string | null>(null)
  ...
  async function run() {
    ...
    setNotice('')
    try {
      // Store the non-secret caller key before sending; a reload or lost response
      // must recover the same durable job instead of creating another assignment.
      const key = pendingJobKey.current ?? window.sessionStorage.getItem(pendingJobStorageKey) ?? crypto.randomUUID()
      window.sessionStorage.setItem(pendingJobStorageKey, key)
      pendingJobKey.current = key
      const assignment = await post<Job>(`${base}/jobs`, { room_id: roomId, idempotency_key: key })
      if (!isCurrent(generation)) return
      window.sessionStorage.removeItem(pendingJobStorageKey)
      pendingJobKey.current = null
      ...
      if (isCurrent(generation)) {
        closePopup(popup)
        setError('Could not confirm job creation. Retry to recover the same job.')
      }
    } finally {
