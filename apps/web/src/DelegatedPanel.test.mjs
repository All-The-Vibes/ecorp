...
async function start(h) {
  ...
  const request = JSON.parse(h.calls.at(-1).init.body)
  assert.match(request.idempotency_key, /^[0-9a-f-]{36}$/)
  assert.deepEqual(request, { actor_id: 'alice', room_id: 'room-a', idempotency_key: request.idempotency_key })
...
test('failed cancellation restores active controls only after a fresh durable read', async () => {
  const h = harness()
  await h.answer(snapshot([operation()]))
  h.click('Cancel job')
  h.calls.at(-1).response.reject(new Error('offline'))
  await h.flush()
  assert.ok(!h.button('Resume sign-in'))
  assert.match(h.calls.at(-1).path, /\?actor_id=alice$/)
  await h.answer(snapshot([operation()]))
  assert.ok(h.button('Resume sign-in'))
  assert.ok(!h.button('Cancel job').props.disabled)
  assert.equal(h.calls.filter((call) => call.path.endsWith('/authorize')).length, 0)
  h.click('Resume sign-in')
  await h.answer({ browser_url: '/api/delegated/browser/recovery-ticket' })
  assert.deepEqual(h.navigations, ['http://localhost:3000/api/delegated/browser/recovery-ticket'])
  h.unmount()
})

test('a read started before cancellation settles cannot restore active controls', async () => {
  const h = harness()
  await h.answer(snapshot([operation()]))
  h.click('Refresh status')
  const oldRead = h.calls.at(-1)
  h.click('Cancel job')
  h.calls.at(-1).response.reject(new Error('response lost'))
  await h.flush()
  oldRead.response.resolve(snapshot([operation()]))
  await h.flush()
  assert.ok(!h.button('Resume sign-in'))
  h.click('Refresh status')
  await h.answer(snapshot([operation()]))
...
test('recovering from cancellation never revives an earlier authorization response', async () => {
  const h = harness()
  await start(h)
  const oldAuthorization = h.calls.at(-1)
  h.click('Cancel job')
  ...
  assert.equal(h.calls.filter((call) => call.path.endsWith('/authorize')).length, 1)
  assert.ok(!h.button('Resume sign-in').props.disabled)
  h.click('Resume sign-in')
  await h.answer({ browser_url: '/api/delegated/browser/new-ticket' })
  assert.deepEqual(h.navigations, ['http://localhost:3000/api/delegated/browser/new-ticket'])
  h.unmount()
})

test('an ambiguous cancellation response follows the durable terminal result without retry', async () => {
  for (const status of ['cancelled', 'expired', 'failed', 'completed']) {
    const h = harness()
    await h.answer(snapshot([operation()]))
    h.click('Cancel job')
    h.calls.at(-1).response.reject(new Error('response lost after commit'))
    await h.flush()
    await h.answer(snapshot([operation({ status })]))
    assert.ok(!h.button('Resume sign-in'))
    assert.ok(!h.button('Cancel job'))
    await h.poll(snapshot([operation({ status })]))
    assert.equal(h.calls.filter((call) => call.path.endsWith('/cancel')).length, 1)
    assert.equal(h.calls.filter((call) => call.path.endsWith('/authorize')).length, 0)
    h.unmount()
  }
})

test('cancelling before automatic sign-in permanently consumes that automatic attempt', async () => {
  const h = harness()
  await h.answer(snapshot())
  h.click('Run protected resource job')
  ...
  assert.equal(h.popups[0].closed, true)
  h.click('Resume sign-in')
  await h.answer({ browser_url: '/api/delegated/browser/manual-recovery' })
  assert.deepEqual(h.navigations, ['http://localhost:3000/api/delegated/browser/manual-recovery'])
  h.unmount()
})
...
test('lost job response and page reload reuse the same caller key until admission is confirmed', async () => {
  const storage = new Map()
  const first = harness({ storage })
  await first.answer(snapshot())
  first.click('Run protected resource job')
  const request = JSON.parse(first.calls.at(-1).init.body)
  first.calls.at(-1).response.reject(new Error('response lost after server commit'))
  await first.flush()
  assert.equal(storage.size, 1)
  first.click('Run protected resource job')
  assert.deepEqual(JSON.parse(first.calls.at(-1).init.body), request)
  first.calls.at(-1).response.reject(new Error('response lost again'))
  await first.flush()
  first.unmount()
  const reloaded = harness({ storage })
  await reloaded.answer(snapshot())
  reloaded.click('Run protected resource job')
  assert.deepEqual(JSON.parse(reloaded.calls.at(-1).init.body), request)
  await reloaded.answer({ mission_id: 'mission-1', task_id: 'task-1' })
  assert.equal(storage.size, 0, 'confirmed admission clears the pending key')
  reloaded.unmount()
})

test('pending job keys remain isolated by server, Corp, room and actor', async () => {
  for (const changed of [{ serverUrl: 'http://other.local:3000' }, { corpId: 'corp-b' },
    { roomId: 'room-b' }, { actorId: 'bob' }]) {
    const storage = new Map()
    const h = harness({ storage })
    const original = h.props
    await h.answer(snapshot())
    h.click('Run protected resource job')
    const firstKey = JSON.parse(h.calls.at(-1).init.body).idempotency_key
    h.calls.at(-1).response.reject(new Error('response lost'))
    await h.flush()
    h.render({ ...original, ...changed })
    await h.answer(snapshot())
    h.click('Run protected resource job')
    assert.notEqual(JSON.parse(h.calls.at(-1).init.body).idempotency_key, firstKey)
    h.calls.at(-1).response.reject(new Error('second scope response lost'))
    await h.flush()
    h.render(original)
    await h.answer(snapshot())
    h.click('Run protected resource job')
    assert.equal(JSON.parse(h.calls.at(-1).init.body).idempotency_key, firstKey)
    h.unmount()
  }
})
